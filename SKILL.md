...
2. **将 Markdown 转换为微信 HTML（关键步骤）**
   - 微信公众号编辑器**不支持 `<style>` 标签**，必须使用内联样式 HTML
   - 使用 `scripts/markdown_to_wechat_doocs.py`（仓库内多版本转换器中**唯一推荐**的一个，与 `THEME_GUIDE.md` 配套）
   - 主题选择：参考 [THEME_GUIDE.md](THEME_GUIDE.md)，按文章类型从 `orange / purple / green / blue / red / cyan / black / pink` 中选一个
   - 命令：
     ```bash
     python3 scripts/markdown_to_wechat_doocs.py \
       --input drafts/article.md \
       --output drafts/article.html \
       --theme orange   # 按文章主题挑选
     ```
   - 输出的 HTML 才是 `create_draft.py --content` 真正要传的内容
