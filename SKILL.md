...
- 不生成封面：直接跳过 `generate_cover.py` 步骤，`create_draft.py` 不传 `--thumb_media_id` 即可（**`generate_cover.py` 没有 `--no-cover` 参数，不要瞎传**）
...
## 资源索引
- 核心脚本：
  - [scripts/create_article.py](scripts/create_article.py) - **遗留 orchestrator（不推荐 agent 默认使用）**：一行命令跑通"提取/创作 + 封面 + 草稿"，参数 `--mode rewrite|create|direct`。**已知不足：默认 provider=doubao（应是 qwen）、跳过 markdown→HTML 内联样式转换、跳过三遍审校**。仅在用户明确要求"快速一键生成、不需要审校"时使用；正常流程请按本 SKILL.md 流程 B 逐步调用 generate_cover / markdown_to_wechat_doocs / create_draft
...
- AppID 和 AppSecret 必须正确，否则无法获取 access_token
